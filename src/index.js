...
function formatDependency(dependency) {
    switch (dependencyFormat.toLowerCase()) {
        case 'gradlekts':
        case 'gradle':
            return formatDependencyGradle(dependency);
            break;
        case 'gradlegroovy':
...
function formatDependencyGradle(depencency) {
    return `compile group: '${depencency.groupId}', name: '${depencency.artifactId}', version: '${depencency.versions[0]}'`;
}
        ...
        inquirer.prompt([
            {
                ...
                "choices": [
                    ...
                    {
                        "name": "Copy to clipboardGradle",
                        "value": "copyToClipboardGradle"
                    },
                    {
